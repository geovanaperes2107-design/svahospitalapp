...
const Reports: React.FC<ReportsProps> = ({ patients, initialReportTab, atbCosts, setAtbCosts, patientDays, setPatientDays }) => {
  ...
  const stats = useMemo(() => {
    ...
    const vencidosCount = filteredPatients.filter(p => p.antibiotics.some(a => a.status === AntibioticStatus.EM_USO && getDaysRemaining(calculateEndDate(a.startDate, a.durationDays)) <= 0)).length;
