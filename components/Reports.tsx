  ...
  const exportToPDF = () => {
    ...
    if (activeReportTab === 'pendentes') {
      ...
      rows = pendingEvaluationPatients.map(p => {
        ...
        return [p.name, p.bed || 'S/L', p.sector, p.diagnosis || 'Não informado', atbStr, activeAtbs[0]?.startDate || '-', activeAtbs[0] ? `D${getATBDay(activeAtbs[0].startDate, activeAtbs[0].frequency)}` : 'D1', 'PENDENTE'];
      });
                    ...
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 px-3 py-1 rounded-lg font-black text-[10px]">
                          D{p.antibiotics[0] ? getATBDay(p.antibiotics[0].startDate, p.antibiotics[0].frequency) : 1}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                    ...
                    p.antibiotics.filter(a => a.status === AntibioticStatus.EM_USO).map(a => {
                      ...
                      return (
                        <tr key={`${p.id}-${a.id}`} className="hover:bg-slate-100/80 dark:hover:bg-slate-900/50 transition-colors border-b border-slate-100 dark:border-slate-700">
                          ...
                          <td className="px-6 py-3 text-center"><span className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-3 py-1 rounded-xl font-black text-[9px]">D{getATBDay(a.startDate, a.frequency)}</span></td>
                    ...
                    {filteredPatients.map(p =>
                      p.antibiotics.map(a => (
                        <tr key={`${p.id}-${a.id}`} className="hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors border-b border-slate-50 dark:border-slate-700">
                          ...
                          <td className="px-6 py-4 text-center font-black text-slate-800 dark:text-slate-200">D{getATBDay(a.startDate, a.frequency)}</td>
