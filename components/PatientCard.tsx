...
const COMMON_RESISTANCE_PROFILES = [
  ...
  'MRSA (Staph. aureus Resistente à Meticilina)',
...
const PatientCard: React.FC<PatientCardProps> = ({ patient, role, activeTab, onUpdate, onDelete, onMoveUp, onMoveDown, canMoveUp, canMoveDown, isDarkMode, onDragStart, onDragOver, onDrop, onDragEnd, isDragging, isDragOver, configAtbDayLock, configAtbDayChangeTime, configAtbDayChangeTimeUTI, activeSectors }) => {
  ...
  const isScirasPanel = activeTab === 'sciras';
  ...
  useEffect(() => {
    ...
    setTempResistanceProfile(patient.resistanceProfile || '');
  ...
  const handleSaveAction = () => {
    ...
    if (editMode?.type === 'novo') {
      ...
      const atbObj: Antibiotic = {
        ...
        startDate: newAtb.start || format(new Date(), 'yyyy-MM-dd'),
      ...
      if (editMode.atbId) {
        ...
        const atbLog = `ATB ${atbToUpdate?.name} editado (Dose/Horário/Via/Início)`;
              ...
              {(patient.microorganism || patient.resistanceProfile) && (
                <div className="flex flex-wrap gap-1">
                  {patient.microorganism && (
                    <span className="px-1 py-0.5 rounded text-[7px] md:text-[8px] font-black uppercase bg-amber-100 text-amber-800 border border-amber-200" title={`Microrganismo: ${patient.microorganism}`}>
                      🧫 {patient.microorganism}
                    </span>
                  )}
                  {patient.resistanceProfile && (
                    <span className="px-1 py-0.5 rounded text-[7px] md:text-[8px] font-black uppercase bg-red-100 text-red-700 border border-red-200" title={`Perfil de Resistência: ${patient.resistanceProfile}`}>
                      🧬 {patient.resistanceProfile}
                    </span>
                  )}
                </div>
            ...
            <div key={atb.id} className="rounded-xl p-2 border border-black/5 bg-white/60 hover:bg-white/80 transition-all flex flex-col gap-2 shadow-inner">
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <button onClick={() => {
                      setNewAtb({
                        ...
                        dose: atb.dose,
                        freq: atb.frequency,
                        times: atb.times.join(', '),
                        duration: atb.durationDays,
                        start: format(new Date(), 'yyyy-MM-dd'),
                        justification: '',
                        route: atb.route || 'EV'
                      });
                      setEditMode({ type: 'troca', atbId: atb.id, oldAtbName: atb.name });
                    }} className="p-1.5 bg-slate-800 text-white rounded-lg shadow hover:scale-105 transition-all border border-slate-700" title="Substituir">
                      <Dna size={14} />
                    </button>
                    {!isCC && (
                      <button onClick={() => {
                        setNewAtb({
                          ...
                          duration: atb.durationDays,
                          times: atb.times.join(', '),
                          start: format(parseAnyDate(atb.startDate), 'yyyy-MM-dd'),
                          route: atb.route || 'EV'
                        });
                        setEditMode({ type: 'tempo', atbId: atb.id });
                      }} className="p-1.5 bg-blue-500 dark:bg-blue-600 text-white rounded-lg shadow hover:scale-105 transition-all" title="Alterar Tempo">
                        <Clock size={14} />
                      </button>
                    )}
                    <button onClick={() => {
                      ...
                      setEditMode({ type: 'editar', atbId: atb.id });
                    }} className="p-1.5 bg-white border border-slate-200 text-slate-400 rounded-lg hover:text-slate-800 shadow-sm transition-colors" title="Editar">
                      <Edit3 size={14} />
                    </button>
                  </div>
                )}
              ...
              <div className={`grid grid-cols-3 ${isCC ? 'md:grid-cols-5' : 'md:grid-cols-6'} gap-2`}>
                <div className="bg-white p-1 rounded-lg shadow-sm border border-black/5 text-center flex flex-col items-center justify-center">
                  ...
                  <input
                    type="date"
                    disabled={isCC || isInfectoPanel}
                    className="w-full bg-slate-50 border border-slate-200 rounded text-center text-[10px] font-black py-0 px-0.5 outline-none focus:border-blue-500 text-slate-800 disabled:opacity-50 cursor-pointer"
                    value={format(parseAnyDate(atb.startDate), 'yyyy-MM-dd')}
                    onChange={e => {
                      const val = e.target.value;
                      if (val) {
                        const updatedAtbs = patient.antibiotics.map(a => a.id === atb.id ? { ...a, startDate: val } : a);
                        onUpdate({
                          ...patient,
                          antibiotics: updatedAtbs,
                          history: addHistory('Alterar Início', `Data de início do ATB ${atb.name} alterada para ${format(parseAnyDate(val), 'dd/MM/yyyy')}`)
                        });
                      }
                    }}
                  />
        ...
        {showMenu && (
          <div className="mt-6 space-y-6 animate-in slide-in-from-top-3 duration-300 text-left border-t border-black/5 pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white/80 p-6 border border-black/5 shadow-sm rounded-2xl space-y-4">
                <label className="text-sm font-black uppercase text-amber-600 flex items-center gap-2"><Bug size={18} /> Painel SCIRAS / CCIH</label>
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-400">Microrganismo Isolado</label>
                    <input
                      type="text"
                      className="w-full bg-slate-50 p-3 rounded-xl font-bold text-xs border-0 outline-none text-slate-800 focus:ring-2 focus:ring-amber-200"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-slate-400">Perfil de Resistência</label>
                    <input
                      type="text"
                      className="w-full bg-slate-50 p-3 rounded-xl font-bold text-xs border-0 outline-none text-slate-800 focus:ring-2 focus:ring-amber-200"
                      placeholder="Ex: KPC, ESBL, MRSA, Sensível..."
                      value={tempResistanceProfile}
                      onChange={e => setTempResistanceProfile(e.target.value)}
                    />
                  </div>
            ...
            <div className="space-y-3">
              ...
              {(editMode.type === 'novo' || editMode.type === 'troca' || editMode.type === 'tempo' || (editMode.type === 'editar' && editMode.atbId)) && (
                <div className="space-y-0.5">
                  ...
                  <input
                    type="date"
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-2 rounded-xl font-bold text-xs outline-none focus:border-blue-500 text-slate-900 dark:text-white"
                    value={newAtb.start ? format(parseAnyDate(newAtb.start), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd')}
                    onChange={e => setNewAtb({ ...newAtb, start: e.target.value })}
                  />
                </div>
              )}
